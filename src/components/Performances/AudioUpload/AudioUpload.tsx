...
class AudioUpload extends React.Component<IAudioUploadProps, IAudioUploadState> {
  ...
  public uploadHandler = async (performanceId: number, update: boolean) => {
    ...
    if (update) {
      for (let i = 0; i < this.state.speeches.length; i++) {
        ...
        if (!speech.isNew) {
          await API.put(`http://10.26.0.230:5000/api/speech/${speech.id}`, speech, {
            headers: {
          ...
          await API.post("http://10.26.0.230:5000/api/speech", speech, {
            headers: {
              "Accept": "application/json",
              "Content-Type": "application/json",
              "Access-Control-Allow-Origin": "*",
  ...
  public async audioComponentDidMount(performanceId: number) {
    ...
    if (performanceId !== -1) {
      const performanceSpeechesResponse = await API.get("http://10.26.0.230:5000/api/performance/" + performanceId + "/speeches");
      ...
      const files = [];
      for (let i = 0; i < performanceSpeechesResponse.data.length; i++) {
        const audiosToSpeech = await API.get("http://10.26.0.230:5000/api/speech/" + performanceSpeechesResponse.data[i].id + "/audios");
      ...
      const languagesResponse = await API.get("http://10.26.0.230:5000/api/language");
    ...
    for (let j = 0; j < files.length; j++) {
      if (files[j].speechIndex === this.state.speeches[speechIndex].id) {
        if (method === HttpMethods.POST) {
          ...
          await API.post("http://10.26.0.230:5000/api/audio", item, {
            headers: {
              "Accept": "application/json",
              "Content-Type": "application/json",
              "Access-Control-Allow-Origin": "*",
          ...
          if (item.id === undefined) {
            ...
            await API.put("http://10.26.0.230:5000/api/audio/" + item.id, item, {
              headers: {
                "Accept": "application/json",
                "Content-Type": "application/json",
                "Access-Control-Allow-Origin": "*",
          ...
          const audiosToSpeech = await API.get("http://10.26.0.230:5000/api/speech/" + item.speechId + "/audios");
          ...
          for (const file of audiosToSpeech.data) {
            if (noChoosedLangId.find((i) => i === file.languageId)) {
              await API.delete(`http://10.26.0.230:5000/api/audio/${file.id}`, {
                headers: {
                  "Accept": "application/json",
                  "Content-Type": "application/json",
                  "Access-Control-Allow-Origin": "*",
  ...
  private onBatchAddItem = async (file: File) => {
    ...
    await API.post("http://10.26.0.230:5000/api/audio/upload", formData, {
      headers: {
    ...
    if (neededSpeech !== undefined) {
      if (this.state.performanceId === -1) {
        ...
        if (!neededSpeech.isNew) {
          const removeResponse = await API.delete("http://10.26.0.230:5000/api/speech/" + index);
  ...
  private unloadFilesAsync = async (mustToBeDeletedAudios: string[]) => {
    if (mustToBeDeletedAudios.length > 0) {
      ...
      await API.delete("http://10.26.0.230:5000/api/audio/unload/" + query);
    }
  }
